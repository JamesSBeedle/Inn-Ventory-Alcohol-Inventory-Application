...
def select_all():
    ...
    for row in results:
        ...
        product = Product(row["name"], row["category"], row["in_stock"], row["cost_price"], row["sale_price"], row["description"], row["minimum_stock_level"], supplier, row["id"])
...
def select(id):
    ...
    if result is not None:
        ...
        product = Product(result["name"], result["category"], result["in_stock"], result["cost_price"], result["sale_price"], result["description"], result["minimum_stock_level"], supplier, result["id"])
    return product
